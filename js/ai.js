...
const AI = (() => {
  ...
  function makeAI(pi, diffKey) {
    ...
    /* my base clusters — owned buildings grouped by proximity. New construction
       spreads across them (and onto fresh ground) instead of stacking on the first. */
    let cluT = -99, cluCache = [];
    function clusters() {
      if (game.t - cluT < 6) return cluCache;
      cluT = game.t;
      const out = [];
      for (const b of myBuildings()) {
        if (b.key === 'gatdef' || b.key === 'artdef') continue;   // defenses don't found towns
        let hit = null;
        for (const c of out) if (U.dist(c.x, c.y, b.x, b.y) < 850) { hit = c; break; }
        if (hit) {
          hit.n++;
          hit.x += (b.x - hit.x) / hit.n; hit.y += (b.y - hit.y) / hit.n;
          hit.keys[b.key] = (hit.keys[b.key] || 0) + 1;
        } else out.push({ x: b.x, y: b.y, n: 1, keys: { [b.key]: 1 } });
      }
      cluCache = out;
      return out;
    }

    /* ground for a forward base: live supplies, well clear of what I already hold,
       and not parked on the enemy's doorstep */
    function expansionSite() {
      let best = null, bestScore = 0;
      for (const d of world.docks) {
        if (d.amount < d.max * 0.2) continue;
        let near = Infinity;
        for (const c of clusters()) near = Math.min(near, U.dist(c.x, c.y, d.x, d.y));
        if (near < 1100 || near === Infinity) continue;      // already covered by a town
        let foeD = Infinity;
        for (const e of game.ents) {
          if (e.dead || e.kind !== 'building' || e.key !== 'cc' || !isEnemyEnt(e)) continue;
          foeD = Math.min(foeD, U.dist(e.x, e.y, d.x, d.y));
        }
        if (foeD < 1500) continue;                            // too deep in their half
        const score = d.amount / 1200 + near / 500;
        if (score > bestScore) { bestScore = score; best = d; }
      }
      return best;
    }

    /* how many of a structure the treasury justifies. Money that sits in the bank
       loses wars, so every wealth tier buys another building. */
    function affordCount(base, per, cap) {
      return Math.min(cap, base + Math.floor(Math.max(0, p().money) / per));
    }
    ...
    function wantedStructure() {
      ...
      // opening build order — one of each core structure before anything scales up
      ...
      if (has('gatdef') < 1 && money >= BUILDINGS.gatdef.cost + buf(500) && game.t > 130) return 'gatdef';
      if (has('artdef') < 1 && money >= BUILDINGS.artdef.cost + buf(700) && game.t > 190) return 'artdef';
      ...
      /* Wealth-scaled build-out: pick whichever structure is furthest below what the
         ...
         together instead of hoarding cash or spamming one thing. */
      const plan = [
        { key: 'supply',    want: affordCount(2, 26000, 14), t: 150, extra: 500,  w: 1.4 },
        ...
        { key: 'power',     want: affordCount(2, 30000, 20), t: 300, extra: 800,  w: 0.5 },
      ];
      let pick = null, bestScore = 0;
      for (const e of plan) {
        if (game.t < e.t || !F.buildings.includes(e.key)) continue;
        const n = has(e.key);
        if (n >= e.want) continue;
        if (money < BUILDINGS[e.key].cost + buf(e.extra)) continue;
        if (e.key === 'supply' && !nearestDockWithSupplies(world.pw / 2, world.ph / 2)) continue;
        if (e.key === 'cc' && !expansionSite()) continue;   // only expand onto real ground
        // FRACTIONAL deficit: score by how far behind its own target a category is,
        // not by raw count. Otherwise the categories with the biggest ceilings
        // (defenses) starve everything else and the AI ends up with 37 turrets,
        // one barracks and one airfield.
        const score = ((e.want - n) / e.want) * e.w;
        if (score > bestScore) { bestScore = score; pick = e.key; }
      }
      if (pick) return pick;
    ...
    /* Which of my holdings most needs another gun? Every town, every forward
       expansion and every control zone I care about is a candidate, scored by how
       little cover it already has. This is what pushes defenses out across the map
       instead of ringing the capital over and over. */
    function defenseAnchor() {
      const mine = myBuildings().filter(b => b.key === 'gatdef' || b.key === 'artdef');
      const assets = [];
      for (const c of clusters()) assets.push({ x: c.x, y: c.y, weight: 1 + c.n * 0.35, r: 620 });
      for (const z of game.zones) {
        // fight for map control: zones I hold or contest deserve their own guns
        const holding = z.owner === myTeam() || (z.present && z.present.includes(myTeam()));
        if (holding) assets.push({ x: z.x, y: z.y, weight: 2.2, r: 560 });
      }
      if (!assets.length) return baseAnchor();
      let best = null, bestNeed = -1;
      for (const a of assets) {
        let cover = 0;
        for (const d of mine) if (U.dist(a.x, a.y, d.x, d.y) < a.r) cover++;
        const need = a.weight / (1 + cover);
    ...
    function findPlacement(key) {
      ...
      let bx = ax, by = ay, maxR = 16;
      if (key === 'gatdef' || key === 'artdef') {
        // guns go wherever my territory is thinnest — a town, an outpost or a zone
        const da = defenseAnchor() || anchor;
        const spot = defenseSpot(key, def, da);
        if (spot) return spot;
        bx = Math.floor(da.x / TILE); by = Math.floor(da.y / TILE);   // arc full: fill in nearby
      } else if (key === 'supply') {
        const dock = nearestDockWithSupplies(anchor.x, anchor.y);
        if (dock) { bx = Math.floor(dock.x / TILE) - 4; by = Math.floor(dock.y / TILE) - 4; }
      } else if (key === 'cc') {
        // found a forward town on fresh supplies rather than a second capital at home
        const site = expansionSite();
        if (site) { bx = Math.floor(site.x / TILE) - 3; by = Math.floor(site.y / TILE) - 3; maxR = 10; }
      } else {
        /* Production and economy spread across every town I own: pick the settlement
           with the fewest of this kind, so expansions grow their own factories and
           airfields instead of everything piling into the capital. */
        const cs = clusters();
        if (cs.length) {
          let bestC = null, bestV = Infinity;
          for (const c of cs) {
            const v = (c.keys[key] || 0) * 10 - c.n * 0.4;
            if (v < bestV) { bestV = v; bestC = c; }
          }
          if (bestC) { bx = Math.floor(bestC.x / TILE); by = Math.floor(bestC.y / TILE); }
        }
      }
      /* Pass 0 demands a free lane around the building and ranges WIDE — as a base
         fills up, the right answer is to build further out, not to wedge structures
         into the gaps and trap your own units. Pass 1 only cramps as a last resort. */
      for (let pass = 0; pass < 2; pass++) {
        const rMax = pass === 0 ? maxR + 14 : maxR;
        for (let r = 1; r <= rMax; r++) {
          for (let attempt = 0; attempt < 14; attempt++) {
            ...
            // even on the fallback path, guns keep their spacing — a stack of turrets
            // on one tile covers no more ground than a single one
            if (key === 'gatdef' || key === 'artdef') {
              const wx = (tx + def.size / 2) * TILE, wy = (ty + def.size / 2) * TILE;
              let clash = false;
              for (const m of myBuildings()) {
                if (m.key !== 'gatdef' && m.key !== 'artdef') continue;
                if (U.dist(m.x, m.y, wx, wy) < 180) { clash = true; break; }
              }
              if (clash) continue;
            }
            return { tx, ty };
    ...
    function manageDozers() {
      ...
      const cc = myBuildings('cc')[0];
      // a builder-less AI is a dead AI: rebuild dozers with zero hesitation, and a rich
      // one runs a whole construction corps so the war chest turns into buildings
      const m = p().money;
      const wantDozers = m > 150000 ? 10 : m > 60000 ? 8 : m > 30000 ? 6 : m > 12000 ? 4 : m > 5000 ? 3 : 2;
      ...
      for (const d of dozers) if (d.order.type === 'build') enRoute.add(d.order.targetId);
      // a site nobody can reach would freeze construction forever — refund and clear it.
      // Sites with a builder walking to them get far longer: a forward expansion on a
      // colossal map is a long march, and killing it mid-journey blocks all expansion.
      for (const site of sites) {
        const patience = enRoute.has(site.id) ? 180 : 75;
        if (site._aiProg !== site.buildProgress) { site._aiProg = site.buildProgress; site._aiProgT = game.t; }
        else if (game.t - (site._aiProgT ?? game.t) > patience) {
      ...
      const maxSites = Math.max(2, Math.min(dozers.length,
        m > 150000 ? 10 : m > 60000 ? 8 : m > 30000 ? 6 : m > 12000 ? 4 : 3));
    ...
    function manageEconomy() {
      for (const sc of myBuildings('supply')) {
        ...
        // every supply centre earns its own haulers, so expansions actually pay for themselves
        const wanted = Math.min(18, 3 * nSup + (posture() === 'steady' || posture() === 'leading' ? 0 : 1));
